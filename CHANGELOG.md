...
In flight. Adds the NC-dedup post-collapse clustering pass driven by Mike's parity report. `--nc-dedup` folds many WPA*02* lines that share the same EAPOL body and MIC and differ only in the trailing bytes of the nonce into one survivor tagged with `FLAG_NC` (`0x80`), so hashcat's default `--nonce-error-corrections=8` recovers every observed nonce from one representative line by iterating `+/- 4` on the trailing byte at MIC-verify time. Default `wpawolf` (no flag) output is byte-identical to v0.3.6 -- the clustering pass is gated behind the new flag.
