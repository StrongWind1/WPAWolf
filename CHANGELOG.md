...
- **`--nc-dedup` clustering pass (post-collapse).** Cluster scope: pairs share `(AP, STA, EAPOL frame, MIC, combo_type)` and the first 28 bytes of the nonce; the trailing 4 bytes (interpreted as a `u32` in both endiannesses; the larger collapse wins, ties go to LE) must fit within `--nc-tolerance` (default 8, matches hashcat's `NONCE_ERROR_CORRECTIONS=8`). Survivor is the sorted-median observed nonce in the cluster, so hashcat's symmetric `[survivor - N/2, survivor + N/2]` iteration covers the full cluster span when the cluster fits in `[0, N]`. Survivor's `message_pair` byte gains `FLAG_NC | FLAG_LE` or `FLAG_NC | FLAG_BE`; the remaining cluster members are dropped. Singleton buckets pass through untouched -- hashcat NC iteration is wasted CPU when no other observed nonce sits within tolerance. Per [IEEE 802.11-2024] §12.7.2 NOTE 9, RC is a performance optimization rather than a security primitive, so merging near-identical nonces does not violate the protocol. See `ARCHITECTURE.md §5.8.1`.
...
- New tests: 10 unit tests in `src/pair/nc_dedup.rs` covering the disabled passthrough, LE / BE cluster detection, span-exceeds-tolerance split, mixed cluster + singleton, no-merge-across-EAPOL-frame, no-merge-across-combo, sorted-median survivor selection, and the Mike-shape 26-nonce stats triple; 3 integration tests in `tests/integration/nc_dedup_collapse.rs` covering the default (no flag) regression-proof, `--nc-dedup` collapsing 26 -> 3, and `--nc-tolerance=4` splitting into more clusters; one new strict-fold unit test (`strict_with_explicit_nc_tolerance_preserves_user_value`) on top of the existing strict-tests fleet (now updated to assert `cli.nc_dedup` is on after `--strict`). 825 tests (up from 807 in v0.3.6); `make check-all` passes clean.
