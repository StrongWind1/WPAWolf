...
Single-purpose follow-up to v0.3.3. The garbage-pattern rejection on cryptographic fields stays as-is; the ESSID handling introduced in v0.3.3 is replaced with a non-fatal warning.

- **ESSID handling reverted from rejection to warning.** v0.3.3 applied the same garbage-pattern detector to SSIDs as to nonces / MICs / PMKIDs and dropped matching SSIDs at extract time. That was the wrong call: a repeating-byte or all-`0xFF` SSID is unusual but not invariant of "garbage" the way it is for HMAC outputs, and the cracker may still recover the right PMK from such an SSID. v0.3.4 keeps the spec-driven discard rule untouched (length 0 wildcard, length > 32 spec violation, first byte 0 hidden-network sentinel) and adds a non-fatal warning for SSIDs that pass that gate but contain at least one byte in `0x00..=0x1F` (the full ASCII C0 control range, NUL through US -- every control character). The warning emits a `[essid_control_bytes]` log line carrying the SSID rendered in lowercase hex and bumps `essid_control_bytes_warned`. The SSID itself is stored and emitted unchanged. Counter `garbage_essid_rejected` and the `[invalid_essid]` log category are removed; the `essid_control_bytes_warned` counter and `[essid_control_bytes]` log category are the operator-facing surface for SSID anomalies.
- **Why a PMKID or EAPOL hash gets discarded.** README "Why a hash gets discarded" enumerates every reason the current code drops a candidate hash before it reaches an output file: Key Nonce garbage pattern (NULL on M1/M2/M3, all-`0xFF` on any message, or a `repeat_1` / `repeat_2` / `repeat_4` short-period repeat -- M4 NULL nonce is spec-valid and is **not** discarded); Key MIC garbage pattern on M2/M3/M4 frames (M1 has no MIC); PMKID garbage pattern at every one of the 20 spec-defined extraction sites; missing ESSID at output time. Discarded values are now also rendered in lowercase hex on the matching log line (`nonce_hex=`, `mic_hex=`, `pmkid_hex=`) so an operator can grep the source capture for the exact bytes.
...
- **Garbage-pattern validation on nonces, MICs, and PMKIDs.** Field-level rejection at extract time now catches three new shape categories on top of the previous all-NULL / all-`0xFF` pair: `repeat_1` (every byte is the same value, e.g. all-`0x55` / all-`0xAB`), `repeat_2` (2-byte alternating period like `5555AAAA` or `01020304...`), and `repeat_4` (4-byte period). Real wire bytes from a healthy stack are HMAC outputs / random nonces -- any of these patterns is a firmware stub or test fixture, not crackable material. Applied at every site that feeds the cryptographic hash pipeline: the EAPOL parser (Key Nonce on M1/M2/M3, plus Key MIC on M2/M3/M4 when the Key MIC flag is set) and every PMKID-bearing IE (M1 KDE, M2 RSN IE, AssocReq / ReassocReq, FT / FILS / PASN Auth, FT Action, Probe Request, Beacon, ProbeResp, Mesh Peering, OSEN). New stats counters (`repeat_nonce_rejected`, `repeat_mic_rejected`, `repeat_pmkid_rejected`) surface in the closing banner; the existing `[invalid_nonce]` / `[invalid_mic]` / `[invalid_pmkid]` `--log` categories now also carry the `repeat_*` kind strings, plus a `nonce_hex=` / `mic_hex=` / `pmkid_hex=` lowercase-hex render of the rejected bytes for forensic traceability. Driven by a downstream-cracker report ("produces a lot of invalid MESSAGE PAIRS and PMKIDs") -- the previous all-NULL / all-`0xFF` rejection caught the obvious cases but left the subtle firmware-stub patterns slipping through. **ESSIDs are not garbage-filtered** -- repeating-byte SSIDs are not invariant of "garbage" the way they are for cryptographic fields, and the cracker may still recover the right PMK from such an SSID. The legacy spec-driven gate (length 1-32, first byte non-zero) is unchanged; SSIDs that pass that gate but contain at least one byte in `0x00..=0x1F` (the full ASCII C0 control range, NUL through US -- every control character) now emit a `[essid_control_bytes]` log line carrying the SSID rendered in lowercase hex, and tick the `essid_control_bytes_warned` counter. The SSID itself is stored and emitted unchanged.
