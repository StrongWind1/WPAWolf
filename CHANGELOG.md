...
- **Garbage-pattern validation across nonces, MICs, PMKIDs, and ESSIDs.** Field-level rejection at extract time now catches three new shape categories on top of the previous all-NULL / all-`0xFF` pair: `repeat_1` (every byte is the same value, e.g. all-`0x55` / all-`0xAB`), `repeat_2` (2-byte alternating period like `5555AAAA` or `01020304...`), and `repeat_4` (4-byte period). Real wire bytes from a healthy stack are HMAC outputs / random nonces -- any of these patterns is a firmware stub or test fixture, not crackable material. Applied at every site that feeds the hash pipeline: the EAPOL parser (Key Nonce on M1/M2/M3, plus Key MIC on M2/M3/M4 when the Key MIC flag is set) and every PMKID-bearing IE (M1 KDE, M2 RSN IE, AssocReq / ReassocReq, FT / FILS / PASN Auth, FT Action, Probe Request, Beacon, ProbeResp, Mesh Peering, OSEN). ESSIDs follow a softer rule -- the legacy hcx admission gate (length 1-32, first byte non-zero) plus the same garbage-pattern check applied only to the legacy-survivors. Length floors mean single-byte SSIDs (`"X"`, `"AB"`, `"LAN"`) are never flagged. New stats counters (`repeat_nonce_rejected`, `repeat_mic_rejected`, `repeat_pmkid_rejected`, `garbage_essid_rejected`) surface in the closing banner; new `[invalid_essid]` `--log` category joins the existing `[invalid_nonce]` / `[invalid_mic]` / `[invalid_pmkid]` triplet. Driven by Mike's downstream-cracker report ("produces a lot of invalid MESSAGE PAIRS and PMKIDs") -- the existing all-NULL / all-`0xFF` rejection caught the obvious cases but left the subtle firmware-stub patterns slipping through. Corpus regression on `/root/ALL_CAPS/collected_with_location` (May 2026, 390 MB / 28 files): hash yield drops from 188,264 to 188,254 (10 lines, 0.005%); 13 SSIDs flag as `repeat_*` and route to the new counter.
