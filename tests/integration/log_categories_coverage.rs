...
fn plcp_error_carries_underlying_error_text() {
    ...
    assert!(!lines.is_empty(), "expected at least one [plcp_error] summary line");
    assert!(
        lines.iter().any(|l| l.contains("dlt=1") && l.contains("count=")),
        "expected aggregated [plcp_error] summary with dlt=1; got {lines:?}"
    );
}
